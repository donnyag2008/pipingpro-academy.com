 ...
 *   GET /api/feed?type=awards &scope=&stage=&award_status=&region=&q=
...
export async function onRequestGet({ request, env }) {
  ...
  if (type === 'jobs') {
    ...
    sql = `SELECT id,headline,summary,operator,contractor,project,scope,stage,award_status,bid_deadline,
                  region,country,location,value_usd,value_text,awarded_date,reported_date,source_url,publisher
           FROM awards WHERE status='live'`;
    const eq = (col, p) => { const v = u.searchParams.get(p); if (v) { sql += ` AND ${col}=?`; binds.push(v); } };
    eq('scope', 'scope'); eq('stage', 'stage'); eq('award_status', 'award_status'); eq('region', 'region');
